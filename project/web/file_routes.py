...
def _text_response(status: HTTPStatus, text: str) -> RouteResponse:
...
def _html_response(status: HTTPStatus, html: str) -> RouteResponse:
...
def build_files_response(parsed, *, workspace_root: Path = WORKSPACE_ROOT) -> RouteResponse:
    ...
    try:
        ...
        return _text_response(HTTPStatus.BAD_REQUEST, str(exc))

    if not path.exists():
        return _text_response(HTTPStatus.NOT_FOUND, tr(lang, "path_not_exist"))
...
def build_download_response(parsed, *, workspace_root: Path = WORKSPACE_ROOT) -> RouteResponse:
    ...
    if not path.exists() or path.is_dir():
        return _text_response(HTTPStatus.NOT_FOUND, tr(lang, "file_not_found"))
